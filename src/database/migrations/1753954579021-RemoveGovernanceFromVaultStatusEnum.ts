...
export class RemoveGovernanceFromVaultStatusEnum1753954579021 implements MigrationInterface {
  name = 'RemoveGovernanceFromVaultStatusEnum1753954579021';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // First update any existing records with 'governance' status to 'locked' (or another appropriate status)
    await queryRunner.query(`UPDATE "vaults" SET "vault_status" = 'locked' WHERE "vault_status" = 'governance'`);
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."vaults_vault_status_enum" AS ENUM('draft', 'created', 'published', 'contribution', 'acquire', 'investment', 'locked', 'failed')`
    );
    await queryRunner.query(
