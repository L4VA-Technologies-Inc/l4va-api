...
export class RewardEpochConfigProxy {
  ...
  async createDraft(dto: any): Promise<any> {
    ...
    const { data } = await firstValueFrom(
      this.httpService.post(url, dto, { headers: { 'X-Internal-Service-Token': this.internalToken } })
    );
    return data;
  }
  ...
  async deleteDraft(version: number): Promise<void> {
    const url = `${this.rewardsBaseUrl}/api/v1/rewards/config/${version}`;
    await firstValueFrom(this.httpService.delete(url, { headers: { 'X-Internal-Service-Token': this.internalToken } }));
  }
}
