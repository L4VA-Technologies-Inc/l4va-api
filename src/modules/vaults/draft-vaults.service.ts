...
export class DraftVaultsService {
  ...
  async saveDraftVault(userId: string, data: SaveDraftReq): Promise<any> {
    ...
    try {
      ...
      } else if (data.vaultImage === null || data.vaultImage === undefined) {
        // If vaultImage is explicitly set to null/undefined, remove the relationship
        vaultData.vault_image = null;
      }
      if (ftTokenImg && !hasFtTokenImage) {
        vaultData.ft_token_img = ftTokenImg;
      } else if (data.ftTokenImg === null || data.ftTokenImg === undefined) {
      ...
      } else if (data.acquirerWhitelistCsv === null || data.acquirerWhitelistCsv === undefined) {
